...
export function dossieToHtml(dossie: Dossie, prospect: Prospect, concorrentes: ConcorrenteExibido[], contexto: ContextoItem[] = []): string {
  ...
  const brainTag = d.encaixe.brain_mode === "live" ? "BASE REAL" : d.encaixe.brain_mode === "nativo" ? "BASE (IMPLANTAÇÃO)" : d.encaixe.brain_mode === "local" ? "BASE LOCAL" : d.encaixe.brain_mode === "fixture" ? "BASE RASCUNHO" : "SEM BASE";
...
<body><div class="sheet">
  ...
  <div class="hero">
    ...
    <div class="gauge">
      ...
      <div class="cap"><b>${d.encaixe.brain_mode === "live" ? "Base real" : d.encaixe.brain_mode === "nativo" ? "Base (implantação)" : d.encaixe.brain_mode === "local" ? "Base local" : d.encaixe.brain_mode === "fixture" ? "Base rascunho" : "sem base"}</b>${nv.nivel !== "sem dado" ? ` · ${esc(nv.nota.replace(/^.*?, /, ""))}` : ""}</div>
    </div>
  </div>
  ${stats}
  <div class="sec">
