...
export function nivelEncaixe(e: EncaixeProspect): NivelEncaixe {
  if (e.brain_mode === "none") return { nivel: "sem dado", pct: 0, nota: "sem Brain deste cliente" };
  const n = e.ganchos.length + e.dores.length;
  const base = e.brain_mode === "live" ? "Brain real" : "Brain de rascunho";
  if (n === 0) return { nivel: "fraco", pct: 22, nota: `${base}, sem ganchos/dores claros` };
  if (n <= 2) return { nivel: "médio", pct: 52, nota: `${base}, ${n} sinal(is) de encaixe` };
  if (n <= 4) return { nivel: "forte", pct: 78, nota: `${base}, ${n} sinais de encaixe` };
  return { nivel: "forte", pct: 92, nota: `${base}, ${n} sinais de encaixe` };
}
...
export function gaugeEncaixeSvg(nv: NivelEncaixe): string {
  ...
  return `<svg viewBox="0 0 ${W} ${H}" width="${W}" height="${H}" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="encaixe ${nv.nivel}">
