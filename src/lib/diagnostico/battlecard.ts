...
export async function gerarBattlecard(diag: DiagnosticoConcorrente): Promise<Battlecard> {
  ...
  const brainBloco =
    brain.mode === "none"
      ...
      : `CONTEXTO DA BASE DE CONHECIMENTO (${brain.mode === "live" ? "base de conhecimento real" : brain.mode === "nativo" ? "base de conhecimento da implantação — confirmada por vocês" : brain.mode === "local" ? "base LOCAL da implantação — enxuta, não é o Brain completo" : "rascunho local — confirmar na base de conhecimento"}):\n${brain.context.slice(0, 3500)}`;
