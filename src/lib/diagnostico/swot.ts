...
export async function gerarSwot(diag: DiagnosticoConcorrente): Promise<Swot> {
  ...
  const brainBloco =
    brain.mode === "none"
      ...
      : `CONTEXTO DO BRAIN do nosso cliente (${brain.mode === "live" ? "real" : brain.mode === "nativo" ? "base de conhecimento da implantação" : brain.mode === "local" ? "base local da implantação" : "rascunho local"}):\n${brain.context.slice(0, 3000)}`;
