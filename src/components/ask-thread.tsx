...
/**
 * ASK THREAD — o núcleo da conversa "Pergunte ao Brain/Radar", reusado por DOIS
 * lugares: a tela `/perguntar` (card) e a BOLHA flutuante (`brain-chat.tsx`).
 *
 * Conversa livre sobre o que o Radar sabe (Brain do cliente + itens coletados),
 * SEMPRE com fontes e honesto quando não sabe. Ancorável a UM cliente
 * (`clientName`) — é o "seletor de cliente"; sem âncora, é multi-cliente.
 *
 * Renderiza um FRAGMENTO ([seletor?] + conversa + input) — quem embrulha (card ou
 * painel) provê a borda/altura e o `scrollClass` da área de conversa.
 */
...
/** Sugestões iniciais — específicas ao cliente ancorado, se houver. */
function suggestionsFor(clientName?: string): string[] {
  if (clientName) {
    return [
      `O que a base de conhecimento sabe sobre ${clientName}?`,
      `Qual o movimento mais importante pra ${clientName} agora?`,
      `Que ação você recomenda pra ${clientName} esta semana?`,
    ];
  }
  return [
    "O que os concorrentes lançaram nos últimos dias?",
    "Qual o movimento mais importante agora?",
...
export function AskThread({
  ...
  placeholder = "Pergunte sobre este cliente, movimentos, oportunidades…",
}: {
  clients: string[];
  /** cliente ancorado (o material/Brain ficam só dele). undefined/"" = todos. */
  clientName?: string;
  ...
  return (
    <>
      {showSelector ? (
        <div className="flex items-center gap-2 border-b border-stone-100 px-4 py-2.5 sm:px-5">
          <span className="shrink-0 text-xs font-medium text-stone-400">Sobre:</span>
          <select
            aria-label="Cliente da conversa"
            value={clientName ?? ""}
            onChange={(e) => onClientChange?.(e.target.value)}
            className="min-w-0 flex-1 rounded-md border border-stone-300 bg-white px-2 py-1 text-sm font-medium text-stone-800 focus:border-stone-500 focus:outline-none"
          >
      ...
      <div className={"space-y-4 overflow-y-auto px-4 py-5 sm:px-5 " + scrollClass}>
        {empty ? (
          <Suggestions items={suggestionsFor(clientName)} onPick={(s) => void ask(s)} />
        ) : (
          turns.map((turn, i) =>
            turn.role === "user" ? (
              ...
              <RadarBubble
                ...
                clients={clients}
                defaultClient={clientName || undefined}
              />
            ),
          )
        )}
      ...
      <form onSubmit={onSubmit} className="border-t border-stone-100 px-4 py-3 sm:px-5">
        ...
        <div className="flex items-end gap-2">
          ...
          <button
            ...
            className="min-h-[40px] flex-none rounded-md bg-red-500 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-red-600 active:bg-red-700 disabled:opacity-50"
          >
            {pending ? "…" : "Perguntar"}
          </button>
        </div>
...
function Suggestions({ items, onPick }: { items: string[]; onPick: (question: string) => void }) {
  return (
    <div className="py-6">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {items.map((suggestion) => (
          <button
            ...
            className="rounded-full border border-stone-200 bg-stone-50 px-3.5 py-2 text-left text-sm text-stone-600 hover:bg-stone-100"
          >
            {suggestion}
          </button>
        ))}
  ...
  return (
    <div className="flex justify-start">
      ...
      >
        ...
        <AproveitarActions text={text} pergunta={pergunta} fontes={fontes} clients={clients} defaultClient={defaultClient} />
      </div>
    </div>
  );
}
...
/**
 * "Aproveitar" (F8) — captura uma resposta boa: guarda como relatório no Radar OU
 * manda pro Formare (vira card). O flywheel aplicado ao chat.
 */
function AproveitarActions({
  ...
  const [done, setDone] = useState<null | { kind: "radar" } | { kind: "formare"; url?: string }>(null);
  const [error, setError] = useState<string | null>(null);
  ...
  async function guardarNoRadar() {
    ...
    try {
      const res = await fetch("/api/reports", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "save-from-chat", clientName: client, question: pergunta, answer: text, fontes }),
      });
      if (!res.ok) {
  ...
  async function gerarNoFormare() {
    ...
    try {
      ...
      const savedPayload = (await saved.json().catch(() => null)) as { data?: { id?: string }; error?: string } | null;
      ...
      const payload = (await res.json().catch(() => null)) as { data?: { mode?: string; ok?: boolean; cardUrl?: string }; error?: string } | null;
      if (!res.ok || !payload?.data?.ok) {
        setError(payload?.error ?? "Não deu pra mandar ao Formare.");
        return;
      }
      setDone({ kind: "formare", url: payload.data.mode === "live" ? payload.data.cardUrl : undefined });
    } catch {
      setError("Falha de conexão.");
    } finally {
      setBusy(null);
  ...
  if (done?.kind === "radar") {
    return <p className="mt-3 border-t border-stone-200/70 pt-2.5 text-xs font-medium text-emerald-700">✓ Guardado em Relatórios ({client})</p>;
  }
  if (done?.kind === "formare") {
    return (
      <p className="mt-3 border-t border-stone-200/70 pt-2.5 text-xs font-medium text-emerald-700">
        {done.url ? (
          <>✓ Criado no Formare — <a href={done.url} target="_blank" rel="noreferrer" className="underline">abrir ↗</a></>
        ) : (
    ...
    <div className="mt-3 border-t border-stone-200/70 pt-2.5">
      {!open ? (
        ...
        <div className="flex flex-wrap items-center gap-2">
          {clients.length > 1 ? (
            <select value={client} onChange={(e) => setClient(e.target.value)} className="rounded-md border border-stone-300 bg-white px-2.5 py-1 text-xs text-stone-700">
              {clients.map((c) => (
                <option key={c} value={c}>{c}</option>
              ))}
            </select>
          ) : null}
          <button type="button" onClick={guardarNoRadar} disabled={busy !== null} className="rounded-md border border-stone-300 bg-white px-3 py-1 text-xs font-medium text-stone-700 hover:bg-stone-100 disabled:opacity-50">
            {busy === "radar" ? "Guardando…" : "Guardar no Radar"}
          </button>
          <button type="button" onClick={gerarNoFormare} disabled={busy !== null} className="rounded-md bg-stone-900 px-3 py-1 text-xs font-medium text-stone-50 hover:bg-stone-700 disabled:opacity-50">
